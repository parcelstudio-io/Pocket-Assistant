...
| 1 | Two-channel oscilloscope | ≥50 MHz, single-shot capture, differential-safe speaker measurement method | `OWNED` | 95% need; captures regulator startup/cutoff, I2S, and speaker differential voltage. **Bought 2026-09-21: Rigol DHO802** — 70 MHz, 2 channels + EXT, 12-bit, Single trigger mode, A−B math usable as a VRMS measurement source, two PVP3150 10X probes (Rigol DHO800 data sheet, Oct 2024). Incoming check: probe compensation, self-calibration after warm-up, and whether its BNC shells are earth-referenced on the supplied USB-C adapter. |
| access to 1 | Fast current-capture instrument | Joulescope `JS320`. The originally specified `JS220` (±3 A continuous, 300 kHz bandwidth, 2 MS/s) is **end-of-life as of 2026-09-20**; the `JS320` replaces it and is compatible with `JS220` accessories. Confirm the `JS320`'s exact range, bandwidth, and sample rate against the ≥100 kHz requirement below before ordering | `ARRANGE` | 90% example; DMM averages cannot qualify Wi-Fi/audio millisecond peaks. Confirm expected peak/pulse limits, calibration, grounding, burden, and error before use. |
| access to 1 | Long-duration charge-current logger | Joulescope `JS320`, or remaining `JS220` stock — see the end-of-life note above — configured to record the complete ≤18 h, 100 mA charge without dropped data | `ARRANGE` | 88% example; validate timestamping, storage, 100 mA accuracy, insertion burden, and fail-safe capture before connecting the cell. This is separate from the endpoint-voltage logger. |
| access to 1 or more | Logging host(s) and storage | Host OS supported by the chosen JS320/JS220/Pico/bench-DMM software, with enough independent USB interfaces and storage for a ≥20 h dry run | `ARRANGE` | 90% need; document the entire USB/ground topology, disable sleep/restarts, verify clocks/files/recovery, and keep it out of the controller's USB source path. JS220 sensing is isolated from USB within its rating; TC-08 ground follows the host, so use electrically insulated thermocouple junctions/attachment and continuity-check that no probe creates a DUT-current path. Follow Pico's host-grounding instructions. A computer-hosted 34461A needs its own continuous logging connection. |
...
**Builder instrument decision, 2026-09-21:** no instrument purchases beyond the DHO802. Every remaining `ARRANGE` instrument above must be borrowed, rented, or otherwise accessed; until it is, each gate that names it stays closed unless that gate is formally rewritten here.
...
- [ ] `ARRANGE` — logging host(s), software, USB/ground plan, and storage that
      pass a ≥20 h dry run for the selected JS320/JS220/Pico/DMM combination
...
- [Joulescope JS320 product page](https://www.joulescope.com/products/js320) — the current part;
  [JS220 specifications](https://download.joulescope.com/products/JS220/JS220-K000/description.html),
